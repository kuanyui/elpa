...
(defun eslint-find-exe ()
  (let* ((repo-root (locate-dominating-file (buffer-file-name) "node_modules"))
         (local-eslint (format "%snode_modules/.bin/eslint" repo-root)))
    (if (file-exists-p local-eslint)
        local-eslint
      (executable-find eslint-fix-executable))))
(defun ttt () (interactive) (message (eslint-find-exe)))
